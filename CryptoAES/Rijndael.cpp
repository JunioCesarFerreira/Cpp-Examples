 ...
 */
int RijndaelClass::setupEncrypt(u32 *rk, const u8 *key, int keybits)
{
  int i = 0;
  u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int RijndaelClass::setupDecrypt(u32 *rk, const u8 *key, int keybits)
{
  int nrounds, i, j;
  u32 temp;
...
void RijndaelClass::encrypt(const u32 *rk, int nrounds, const u8 plaintext[16],
  u8 ciphertext[16])
{
...
void RijndaelClass::decrypt(const u32 *rk, int nrounds, const u8 ciphertext[16],
  u8 plaintext[16])
{
  u32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
